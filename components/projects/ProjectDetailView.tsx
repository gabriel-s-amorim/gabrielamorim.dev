...
import {
  ...
  getProjectShowcase,
  type GalleryItem,
} from "@/content/projectGalleries";
import { TechIcon } from "@/components/ui/TechIcon";
import { IconExternal, IconGitHub } from "@/components/ui/icons";
import { ProjectMedia } from "./ProjectMedia";
import { ProjectLightbox } from "./ProjectLightbox";
import { ProjectShowcaseVideo } from "./ProjectShowcaseVideo";
...
export function ProjectDetailView({ project }: ProjectDetailViewProps) {
  ...
  const showcase = useMemo(() => getProjectShowcase(project), [project]);
  const gallery = useMemo(() => {
    const items = getProjectGallery(project);
    // Com walkthrough dedicado, a galeria fica só com screenshots.
    return showcase ? items.filter((item) => item.kind === "image") : items;
  }, [project, showcase]);
      ...
      <div className="lg:hidden">
        ...
        {showcase ? (
          <div className="px-3 sm:px-4">
            <ProjectShowcaseVideo
              ...
              title={project.title}
              durationLabel={showcase.durationLabel}
            />
          </div>
        ) : (
        ...
        <div className="section-container mt-5 space-y-5">
          ...
          <p className="text-xs text-sage-500">
            {showcase
              ? "Toque no vídeo para assistir · miniaturas abrem a galeria"
              : "Deslize a imagem · toque nas miniaturas"}
          </p>
        </div>
      </div>
        ...
        {showcase ? (
          <div className="mt-8 grid gap-12 lg:grid-cols-[1.2fr_0.8fr] lg:items-start">
            <div className="space-y-4">
              ...
              {gallery.length > 0 && (
                <div className="overflow-hidden rounded-3xl border border-moss-700/40 bg-soil-950/60">
                  <div className="flex items-center justify-between border-b border-moss-700/30 px-4 py-3">
                    <p className="text-xs uppercase tracking-wide2 text-sage-400">
                      Screenshots
                    </p>
                    <button
                      type="button"
                      onClick={() => setLightbox(true)}
                      className="text-xs text-sage-400 transition hover:text-amber-300"
                    >
                      Ver galeria
                    </button>
                  </div>
                  <div className="flex gap-2 overflow-x-auto p-3">
                    {gallery.map((item, index) => (
                      <button
                        ...
                        className="relative h-20 w-28 shrink-0 overflow-hidden rounded-xl border border-white/10 opacity-80 transition hover:border-amber-400/50 hover:opacity-100"
